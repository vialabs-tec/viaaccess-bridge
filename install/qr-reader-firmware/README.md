...
| 3 clicks | Force SoftAP `viaaccess-setup` immediately. Join it; the phone “Sign in to network” sheet should open **setup** (Fiação tab when already claimed). Fallback: `http://192.168.4.1/setup` (no port). First-boot captive still opens `/wifi`. Required to change setup after the reader is provisioned (LAN/STA writes are rejected). On first boot the portal stays up until the claim is saved. After provision, a successful `/setup` save (or `/wifi` GOT_IP) closes it. |
